...
if ([string]::IsNullOrEmpty($Features)) {
    cargo build --release --config "target.$TargetTriple.linker=\`"rust-lld\`"" --bin ssurl --bin ssservice --bin ssmanager
}
else {
    cargo build --release --config "target.$TargetTriple.linker=\`"rust-lld\`"" --bin ssurl --bin ssservice --bin ssmanager --features "${Features}"
}
